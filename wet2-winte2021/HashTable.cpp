...
hashTable::hashTable() : playersNum(0), tableSize(START_SIZE), deletedCounter(0), addCounter(0) {
    this->table = new Player [START_SIZE];

    for (int i = 0; i< this->tableSize; i++) {
        this->table[i] = nullptr;
    }
    this->deletedPlayer = new player(0,0,0);
}
...
hashTable::hashTable(hashTable &ht)
    ...
    this->table = new Player [ht.tableSize];
    for (int i = 0; i< this->tableSize; i++) {
...
void hashTable::CleanTable() {
    Player *newTable = new Player [this->tableSize];
    Player * oldTable = this->table;
    this->table = newTable;
...
int hashTable::Find(int playerID) {
    for (int i = 0; i < this->tableSize; i++) {
        if (this->table[i] != nullptr && this->table[i] != this->deletedPlayer &&
            this->table[i]->getPlayerID() == playerID) {
            return i;
        }
    }
    return -1;
...
StatusType hashTable::AddNode(Player player) {
    ...
    for (int i = 0; i < this->tableSize; i++) {
        int plc = this->HashFunc(player->getPlayerID(),i);
...
StatusType hashTable::AddPlayer(int playerID, int groupID,int score ,int Level) {
    if(playerID <= 0) {
    ...
    Player toAdd;
    try{ toAdd = new player(playerID, groupID,score);
    }
    catch (std::bad_alloc& ba) {
        return ALLOCATION_ERROR;
    }
...
int hashTable::GetGroupID(int playerID) {
    ...
    return this->table[index]->getGroupID();
}

int hashTable::GetLevel(int playerID) {
    return this->table[this->Find(playerID)]->getLevel();
}

Player hashTable::GetPlayer(int playerID) {
    int index = this->Find(playerID);
    if (index < 0) {
        return nullptr;
    }
...
int hashTable::GetScore(int playerID) {
    ...
    return this->table[index]->getScore();
}
